...
class Bidir_LSTM_Attention_Model(nn.Module):
    ...
    def __init__(self, n_input=n_input, n_hidden=n_hidden, n_layers=n_layers,
        ...
        # Import and add attention mechanism
    ...
    def forward(self, x, hidden):
        ...
        # Apply attention instead of taking last timestep
    ...
    def init_hidden(self, batch_size):
        """Initialize hidden state on same device as model"""
        weight = next(self.parameters())
        device = weight.device
        
        hidden = (torch.zeros(self.n_layers*2, batch_size, int(self.n_hidden/2), device=device, dtype=weight.dtype),
                  torch.zeros(self.n_layers*2, batch_size, int(self.n_hidden/2), device=device, dtype=weight.dtype))
