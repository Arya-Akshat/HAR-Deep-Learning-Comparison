...
def train(net, X_train, y_train, X_test, y_test, opt, criterion, epochs=100, clip_val=15, device='cpu'):
    print("\n\n********** Running training! ************\n\n")

    sched = getLRScheduler(optimizer=opt)
    # Model already moved to device in main.py
    ...
    for epoch in range(epochs):
        ...
        while step * batch_size <= train_len:
            ...
            inputs, targets = torch.from_numpy(batch_xs).float(), torch.from_numpy(batch_ys.flatten('F')).long()
        ...
        test_loss, test_f1score, test_accuracy, best_accuracy, best_model = test(net, X_test, y_test, criterion, best_accuracy, best_model, test_batch=len(X_test), device=device)
